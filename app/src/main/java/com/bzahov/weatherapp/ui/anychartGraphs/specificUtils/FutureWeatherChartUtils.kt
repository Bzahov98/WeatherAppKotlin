...
import com.bzahov.weatherapp.ui.anychartGraphs.specificUtils.AbstractDayDataChartUtils.Companion.CHART_TYPE_DEFAULT
import com.bzahov.weatherapp.ui.weather.future.list.FutureListState

class FutureWeatherChartUtils/* : AbstractDayDataChartUtils*/{
	companion object {
		private const val TAG = "FutureWeatherChartUtils"
		...
		fun createChart(
			weatherStateData: FutureListState?,
				chartType: String = CHART_TYPE_DEFAULT
		): Cartesian {
			val chartTitle = ForecastApplication.getAppString(R.string.graphs_5days_default_title)

			val cartesian: Cartesian = AnyChartGraphsUtils.setupAndGetBaseCartesian(chartTitle)
...
//            .scale(scalesLinear)
//
			var data: MutableList<DataEntry> = AnyChartGraphsUtils.testChartData() // TODO: Debug
...
//                line.stroke("2 red");
		...
		fun createTemperatureChart(weatherStateData: FutureListState?): Cartesian {
			val chartTitle = ForecastApplication.getAppString(R.string.graphs_5days_temperature_title)
			val cartesian: Cartesian = AnyChartGraphsUtils.setupAndGetBaseCartesian(chartTitle)
			cartesian.yScale().stickToZero(true)
...
//
		...
		fun createPrecipitationsChart(
			...
			val xAxisName = ForecastApplication.getAppString(R.string.graphs_5days_default_axis_x_name)
			val yAxisName =
				ForecastApplication.getAppString(R.string.graphs_precipitation_axis_y_name_precip)
			val series1Name =
				ForecastApplication.getAppString(R.string.graphs_precipitation_series_rain)
			val series2Name =
				ForecastApplication.getAppString(R.string.graphs_precipitation_series_snow)
			val unitAbbreviation =
				ForecastApplication.getAppString(R.string.graphs_precipitation_rain_unit_abbreviation)
			val chartTitle = ForecastApplication.getAppString(R.string.graphs_5days_precipitation_title)
			...
			val percentYAxis =
				yAxis1.title(ForecastApplication.getAppString(R.string.graphs_precipitation_axis_y_namecloud_humid))
...
//                line.stroke("2 red");
			...
			line.name(ForecastApplication.getAppString(R.string.graphs_precipitation_line_cloudiness))
			line.labels().enabled()

			val humidityLine: Line = cartesian.line(humidityLineData)
			humidityLine.name(ForecastApplication.getAppString(R.string.graphs_precipitation_line_humidity))
...
//            pressureLine.name("Pressure, hPa")
			...
			AnyChartGraphsUtils.setupTooltipAndInteractivity(cartesian)

			AnyChartGraphsUtils.setSeriesTooltip(series1, Anchor.RIGHT_BOTTOM)
			AnyChartGraphsUtils.setSeriesTooltip(series2, Anchor.LEFT_BOTTOM)
			AnyChartGraphsUtils.setSeriesTooltip(line, Anchor.CENTER)
			AnyChartGraphsUtils.setSeriesTooltip(humidityLine, Anchor.AUTO)
			//
			...
			AnyChartGraphsUtils.colorizeAxesBackground(xAxis)
		...
		fun createWindChart(
			...
			val xAxisName = ForecastApplication.getAppString(R.string.graphs_5days_default_axis_x_name)
			val yAxisName = ForecastApplication.getAppString(R.string.graphs_wind_axis_y_name_speed)
			val series1Name = ForecastApplication.getAppString(R.string.graphs_wind_series_speed)
			val series2Name = ForecastApplication.getAppString(R.string.graphs_wind_series_degree)
			val unitAbbreviation =
				ForecastApplication.getAppString(R.string.graphs_wind_speed_unit_abbreviation)
			val chartTitle = ForecastApplication.getAppString(R.string.graphs_5days_wind_title)
...
//                line.stroke("2 red");
			...
			line.name(ForecastApplication.getAppString(R.string.graphs_wind_line_speed))
...
//            series2.name(series2Name)
			...
			AnyChartGraphsUtils.setSeriesTooltip(
				series1,
				Anchor.RIGHT_BOTTOM,
				"Wind Speed: {%value}$unitAbbreviation , <br> <b>Wind direction: {%$KEY_WIND_DEGREE}</b>"
			)
			AnyChartGraphsUtils.hideSeriesTooltip(line)
